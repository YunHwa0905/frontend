...
function cb ( data ) {
    ...
    // 필터링 : 30이상(>=) 참, 아니면 거짓 => 조건식(의 결론 boolean)
    return data >= 30;
}
let results3 = arr3.filter( cb );
console.log( arr3, results3 );
// 실습 : 40분에 2분 진행
// 1. 위의 표현을 화살표 함수로 변경하여 filter( 화살표함수 형태 ) 구현하시오
let result4_ = arr3.filter( ( data ) =>{
    return data >= 30; 
});
...
// data % 2 => 짝수면 0으로 해석 => 조건식에 0 => false로 처리됨 => 홀수만 포함됨
...
// 누적합
// pv:이전 값, cv:현재 값
// 통계 <- 대시보드 상 차트 그리기 시 데이터 연산처리할때 사용
