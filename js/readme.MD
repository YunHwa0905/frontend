# Javascript 언어
- 목적
     - 전 세계 언어 top10에 속함 => 개발자들 많이 찾는다
     - 프러트앤드 포지션에서 SPA 개발할 때 주력언어
...
- 개요
     - History
          ...
          - 2005년
               - google map 등장
               - js에 `ajax`(백그라운드 비동기 통신기술, xml 을 대상) 기술 적용 -> 지도 서비스 개시
          - 2005년 이후
     ...
     - 버전
          ...
          - 2009 : ESS, ECMAScript 5, 현재 형태로 개정
          - 2015 : `ES6`, ECMAScript 2015, ECMA2015(라고도 씀), 현재 기본 표준 형태 => `ES Next`
          - 2015 이후 : 매년 문법이 추가되어 ECMA 에서 발표
          - 특징
               - 최신문법 작성된 JS -> 특정시점의 표준으로 변환(ES6), babel -> ES6로 변환되어서 구동

     - JS 표현 범위
          - TS(TypwScript) > ES Next > ESS 
          - 최종 작성된 JS는 ES6로 맞춘다
     ...
     - 식별자 (identifier)
          ...
          - 네이밍 컨벤션 종류 -> 전통, 문화 -> 동일하게 작성
               - 카멜 표기법
                    ...
                    - ex) _name, getName, Person(클래스인 경우)
               - 스네이크 표기법
                    - Python, ..
...
- 배열 -> js 내부에서 정의된 특수 개체
     - 8_array.js
     
- 객체
     - 최신 문법(es next 이후)에서 클래스 표기법이 추가됨
     - 타 언어의 객체와 비슷하지만 다르다 !!
     - 객체지향 프로그래밍 스타일을 따르지만 실제적으로 다르다
     - 데이터 형식을 표방한다( 키:값의 조화 ) => JSON과 매칭된다 => {}
     - 9_object.js
     - 10_class.js
...
- 최신문법 => 코드의 효율선 극대화!!
     ...
     - 비동기 처리
          ...
          - generator, ... <= 복잡
          - 14_async_promise.js
     - 연산시 null/undifined에 대한 오류 대비 처리
          - 15_cal_null_safe.js
