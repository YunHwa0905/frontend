 ...
 *      - 수치, 문자열, 불린 등등 특정 타입의 데이터를 담는 그릇
 *  - 키워드
 *      - var
 *          - 초기부터 계속 사용
 *          - 버그적인 문제가 내제되어 있음
 *      - 표현 추가
 *          - 변수의 범위를 블록 스코프({여기서만 유호})에만 유효
 *          - let
 *              - 변수
 *          - const
 *              - 상수 : 값을 한번 세팅하면 절대로 바꿀수 없다
 *              - 상수명은 반드시(통상) 대문자(_ 추가 사용)로만 표현
 *              - 환경변수, 설정값, CONFIG용도
 */
...
// 1. 상수
const PI = 3.14;
console.log(PI);// 콘솔(터미널) 출력 -> 코드가 잘 작동했는지 결과를 확인

// 상수값 수정 -> 오류 발생 -> 상수값을 변경 시도해서 오류남
...
// 변수가 가지고 있는(가르키고 있는) 데이터를 변경 (다른 타입으로)
a = "hi";
console.log(a,typeof a);

// 변수는 값을 언제든지 바꿀수 있다 !!
// 변수는 값을 들고 잇는 것이 아니라? 가르키고 있는것(참조) 아닌가?
...
if(true){
    ...
    // a라는 변수가 전역과 지역 사이에서 겹친다
    // -> 변수명 통일 -> 같은 변수로 취급 : var
    var a = 10; 
    var b = 11; // 지역(local)변수
    let c = 12; // let은 var의 버그적인 상황 해결
} 
console.log('a= ', a);
...
// 호이스팅 : 증상/현상 -> 인터프린터가 토드 실행전에 함수, 변수, 
//            클래스 등 최상위로 선언문을 배치하는 증상/형상
console.log( a2 );
var a2 = 10;
console.log( a2 );
/*
