/**
 * 코드 전개시(연산시 등) 오류 코드가 발생할 수 있다 -> 방지코드
 *  - ?.
 *      - 예시
 *          - 비동기 처리시 => 함수().then().then()...
 *          - 객체명.멤버.멤버...
 *      - 위와 같은 코드를 옵셔널 체이닝 코드 여기서 앞부분에 오류가 발생하면 중단
...
// 요구사항 : code값 10을 출렷하시오, 1분
// 객체명.멤버
console.log(A.proc.msg.code);
// 원래 데이터에는 check:{ code:5 } 있었다 -> 정상적으로 세팅된 것으로 보고 작성
//console.log(A.proc.msg.check.code); // 셧다운 발생
console.log(A?.proc?.msg?.check?.code); // 안전하게 코드 작성
// undefined 발생 => 대응 가능함 => 조건문
// 해결
// 예외처리
// (*) 방어처리
console.log(1); // 이것이 출력괴면 셧다운 X, 출력 않되면 중단된것임


// 서비스 => 환경변수 => 선택적 값 부여 가능
// 서버 포트 지정 ( 커스텀 || 시스템 설정 포트 )
// 서버 포트 지정 ( 개발 || 시스템 설종 포트 )
