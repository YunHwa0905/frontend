...
// 전역변수
const EXCHANGE_URL = 'http://127.0.0.1:5500/react/app-npm/hook-test/dumy/exchange.json';//'http://api.manana.kr/exchange/rate/KRW/JPY,USD,EUR.json';
// 수치만 입력되게 필터링 처리 함수
// 숫자가 아닌문자를 특정하여 모두 빈값으로 대체
// 숫자 이외에 추가할 기호가 있다면 계속 추가하면 됨 => . 추가
...
function App ( props ) {
    ...
    // 환율 정보 -> 특정 주기로 갱신 -> 화면 갱신에 영향 X -> useRef
    // exchangeRate
    ...
    const getExchangeInfo = useCallback( async ()=>{
        ...
        // 통화 정보 세팅
         /*
                ...
                // 값 추출
                객체명['USD'] => 1432.xx
                실습 : res.data를 전처리 등 조치 => 위의 구조처럼 구성
         */
        // find()은 배열 내에서 특정 정보가 일치하는 요소만 찾는 함수
